...
function draw_one_graph(dateCol,toChart) {
   ...
   if (json_parameters.time_level === 'DAY') {
   ...
   draw_fusion_graph(xDataString, chartYData, chartContainer, "Selected KPIs");
}
...
function draw_multiple_graphs(dateCol,toChart) {
   ...
   for(var i = 0; i < nr_of_graphs; i++) {
      ...
      var ci_alias = json_data_per_id[i].graph_alias;
      ...
      draw_fusion_graph(xDataString, chartYData, chartContainer, ci_alias + ' (' + ci + ')');
   }
}
...
function createChart() {
   ...
   dateCol = get_selected_columns(elements,toChart, json_parameters.column_with_timestamp );
...
function draw_fusion_graph(xDataString, chartYData, chartContainer, chartCaption) {
    // console.log("Chart Caption: " + chartCaption);
