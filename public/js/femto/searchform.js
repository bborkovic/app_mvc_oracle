...
function searchcell() {
   // select dom element, and clean it
   var list_of_cells = document.getElementById("ci-list");
   while (list_of_cells.hasChildNodes()) {
      list_of_cells.removeChild(list_of_cells.lastChild);
   }

   var el = document.getElementById("cell-names");
   var inputtext = el.value.toLowerCase();

   if (inputtext.length >= 2) {
      for (var i = 0; i < json_list_of_cells.length; i++) {
         var cell_name = json_list_of_cells[i].naziv;
         var ci = json_list_of_cells[i].ci;
         if (cell_name.indexOf(inputtext) >= 0) {
            var option_element = document.createElement('option');
            option_element.value = ci;
            option_element.textContent = cell_name + ' ( ' + ci + ' )';
            list_of_cells.appendChild(option_element);
         }
      }
   }
}
...
function populate_meas_classes() {
   meas_classes.sort(function(a, b) {
